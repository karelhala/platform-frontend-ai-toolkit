...
Call the `db-upgrader` skill with the replication-check action:

```javascript
Skill("db-upgrader", args: "{service} {environment} {version} {product} replication-check")
```
...
- `$ARGUMENTS.action` - "replication-check"

The skill will generate the SQL query YAML file at:
`data/app-interface/sql-queries/{product}/{service-short}/{env}/{date}-{env}-check-used-replication-slots.yaml`
